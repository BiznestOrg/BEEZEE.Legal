# Policies

Legal documents for the **BEEZEE** (EasyManagement) platform.

## Documents

| Document | EN | PL | UK |
|----------|----|----|-----|
| Privacy Policy | [English](privacy-policy/en.html) | [Polski](privacy-policy/pl.html) | [Українська](privacy-policy/uk.html) |
| Terms of Use | _Coming soon_ | _Coming soon_ | _Coming soon_ |

## Hosting

These documents are designed to be hosted as a static website on **Azure Storage Static Website**.

## Contributing

All changes must go through pull requests — direct pushes to `main` are not allowed.
Legal documents must be reviewed before publication.
